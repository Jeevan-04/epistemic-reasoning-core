...
class ChittaGraph:
    ...
    def find_matching_belief(self, proposal: dict) -> Belief | None:
        ...
        for bid in candidates:
            ...
            if belief._canonical == canonical:
                return belief
    ...
    def add_unknown(
        ...
        if existing:
            ...
            if new_confidence >= 0.3 and existing.epistemic_state == "unknown":
                existing.epistemic_state = "hypothetical"
    ...
    @classmethod
    def load(cls, filepath: str | Path) -> ChittaGraph:
        ...
        # Load beliefs
        for belief_data in data["beliefs"].values():
            belief = Belief.from_dict(belief_data)
            graph.add_belief(belief)
        
        # Load edges
        for relation, src_dict in data.get("edges", {}).items():
