...
try:
    ...
    from .belief import Belief, Polarity
except ImportError:
    from graph import ChittaGraph
    from belief import Belief, Polarity
...
try:
    ...
    from schemas import SchemaExtractor, RelationSchema
except ImportError:
    SchemaExtractor = None
    RelationSchema = None

# Import RelationFrame for structural predicate typing
try:
    sys.path.insert(0, str(Path(__file__).parent.parent))
    from CORE_TYPES import RelationFrame, RelationKind
except ImportError:
    RelationFrame = None
    RelationKind = None
...
class Buddhi:
    ...
    def decay_beliefs(self):
        ...
        for belief in self.chitta.beliefs.values():
            ...
            if belief.epistemic_state == "hypothetical":
        ...
        for belief in self.chitta.beliefs.values():
            ...
            if belief.epistemic_state == "hypothetical":
                if belief.confidence > PROMOTE_THRESHOLD:
                    # Promote
                    belief.epistemic_state = "asserted"
    ...
    def demote_weak_beliefs(self):
        ...
        for belief in self.chitta.beliefs.values():
            ...
            if belief.epistemic_state == "unknown":
                continue
    ...
    def _apply_taxonomic_inference(
        ...
        """
        Answer queries using taxonomic inheritance (is-a chains).
        
        THE NEGATION DOMINANCE RULE:
        This is probably the most important rule in MARC. Here's the idea:
        
        If you know "penguins are birds" and "birds can fly", you might think
        "penguins can fly" inherits. But if you ALSO know "penguins cannot fly",
        that negative fact blocks the inheritance. Negations win.
        
        The algorithm:
        1. Walk up the is-a chain (penguin → bird → animal)
        2. Check each ancestor for negative facts FIRST
        3. If any negation found → answer is NO (stop immediately)
        4. If no negations found → check for positive facts
        5. If positive found → answer is YES
        
        RELATION FRAME AWARENESS:
        Not all properties inherit the same way:
        - TAXONOMIC (is_a): always inherits
        - FUNCTIONAL (has_gills, produces_milk): inherits, negation blocks
        - SPATIAL (located_in): does NOT inherit
        - STATE (is_liquid): does NOT inherit
        
        Example:
            Query: "Do bats have gills?"
            Facts: "Bats are mammals", "Mammals do not have gills"
            Result: NO (negation on ancestor blocks positive inheritance)
        
        Returns:
            True if we found an answer via taxonomy, False otherwise
        """
        ...
        for pred in query_predicates:
            if RelationFrame:
                ...
                # Fallback: assume all predicates inherit
                inheritable_predicates.add(pred)
        
        if not inheritable_predicates:
            # No inheritable predicates - skip taxonomic inference
            return False
        ...
        for entity in query_entities:
            ...
            # PHASE 1: Check for NEGATIONS first (dominance)
            # If any ancestor has a NEGATIVE belief for this predicate, return NO
            for ancestor, path in ancestors:
                for belief in self.chitta.beliefs.values():
                    ...
                    if ancestor in belief.entities:
                        ...
                        # Only check inheritable predicates
                        matching_preds = inheritable_predicates & belief_preds
                        
                        if matching_preds:
                            # Check if this predicate's frame blocks on negation
                            for pred in matching_preds:
                                if RelationFrame:
                                    frame = RelationFrame.for_predicate(pred)
                                    if not frame.negation_blocks:
                                        continue  # Skip predicates that don't block
                                
                                # Found predicate match - check polarity
                                if belief.polarity == Polarity.NEGATIVE:
                                    # NEGATION DOMINANCE: Stop immediately, return NO
                                    proof.verdict = "no"
                                    chain_str = " → ".join([entity] + path + [ancestor])
                                    
                                    proof.add_step(
                                        rule="negation_dominance",
                                        inputs=path + [belief.id],
                                        output=f"Negation on ancestor blocks inheritance: {chain_str} (predicate: {pred})",
            ...
            # PHASE 2: No negations found - check for POSITIVE inheritance
            for ancestor, path in ancestors:
                for belief in self.chitta.beliefs.values():
                    if not belief.active:
                        continue
                    ...
                    if ancestor in belief.entities:
                        ...
                        if matching_preds:
                            # Only accept POSITIVE beliefs here (negations handled above)
                            if belief.polarity == Polarity.POSITIVE:
                                proof.verdict = "yes"
                                chain_str = " → ".join([entity] + path + [ancestor])
                                ...
                                proof.add_step(
                                    ...
                                    inputs=path + [belief.id],
                                    output=f"Inheritance chain: {chain_str} (predicate: {list(matching_preds)[0]})",
                                    confidence=belief.confidence
                                )
    ...
    def answer(self, query_proposal: dict) -> AnswerProof:
        ...
        for belief in focus_set:
            ...
            entity_overlap = len(query_entities_set & belief_entities) / max(len(query_entities_set), 1) if len(query_entities_set) > 0 else 0
            predicate_overlap = len(query_predicates_set & belief_predicates) / max(len(query_predicates_set), 1) if len(query_predicates_set) > 0 else 0
            
            # STRICT APPLICABILITY: Require EXACT predicate match + high entity overlap
            # This is the restraint gate - only beliefs that directly answer get through
        ...
        # ═══════════════════════════════════════════════════════════════
        # ANSWER PIPELINE: Direct → Taxonomic → Property → Unknown
        # ═══════════════════════════════════════════════════════════════
        
        # Try direct answer from applicable beliefs
        direct_result = self._try_direct_answer(applicable_beliefs, query_proposal, proof)
        if direct_result:
            return direct_result
        
        # Try taxonomic inference
        if self._apply_taxonomic_inference(query_entities_set, query_predicates_set, proof):
            self.stats["taxonomic_inferences"] += 1
    ...
    def _verbalize_uncertain(self, decision: dict, focus_set: list[Belief]) -> str:
        ...
        if focus_set:
            ...
            for b in focus_set:
                if b.confidence > 0.7:
                    general = b
                elif b.confidence < 0.5 and b.epistemic_state == "hypothetical":
                    exceptions.append(b)
